...
class TestDecoderMisc:
    ...
    def test_decoder_repr(self):
        ...
        dec = JSONDecoder(typ)
        expected = f"structtype.json.Decoder({typ!r})"
        assert repr(dec) == expected

        dec = JSONDecoder()
        assert repr(dec) == f"structtype.json.Decoder({Any!r})"
...
class TestLiteral:
    ...
    def test_int_literal_errors(self):
        ...
        with pytest.raises(structtype.ValidationError, match="Invalid enum value 4"):
            dec.decode(b"4")
    ...
    def test_str_literal_errors(self):
        dec = JSONDecoder(Literal["one", "two", "three"])
        ...
        with pytest.raises(
            structtype.ValidationError, match="Invalid enum value 'bad'"
        ):
            dec.decode(b'"bad"')

    def test_bool_literal_true_only(self):
        dec = JSONDecoder(Literal[True])
        assert dec.decode(b"true") is True
        with pytest.raises(
            structtype.ValidationError, match="Invalid enum value False"
        ):
            dec.decode(b"false")

    def test_bool_literal_false_only(self):
        dec = JSONDecoder(Literal[False])
        assert dec.decode(b"false") is False
        with pytest.raises(structtype.ValidationError, match="Invalid enum value True"):
            dec.decode(b"true")

    def test_bool_literal_errors(self):
        dec = JSONDecoder(Literal[True])
        with pytest.raises(
            structtype.ValidationError, match="Expected `bool`, got `int`"
        ):
            dec.decode(b"42")
        with pytest.raises(
            structtype.ValidationError, match="Expected `bool`, got `str`"
            ...
            dec.decode(b'"hello"')
        with pytest.raises(
            structtype.ValidationError, match="Expected `bool`, got `null`"
        ):
            dec.decode(b"null")
...
class TestSequences:
    ...
    def test_decode_fixtuple_any(self):
        ...
        assert res == x
        with pytest.raises(
            structtype.ValidationError, match="Expected `array`, got `int`"
        ):
            dec.decode(b"1")
        with pytest.raises(
            structtype.ValidationError, match="Expected `array` of length 3"
        ):
            dec.decode(b'[1, "two"]')

    def test_decode_fixtuple_typed(self):
        dec = JSONDecoder(tuple[int, str, bool])
        x = (1, "two", False)
        res = dec.decode(b'[1, "two", false]')
        assert res == x
...
class TestDict:
    ...
    def test_decode_dict_literal_key(self):
        dec = JSONDecoder(dict[Literal["a", "b"], int])
        assert dec.decode(b'{"a": 1, "b": 2}') == {"a": 1, "b": 2}

        with pytest.raises(structtype.ValidationError, match="Invalid enum value 'c'"):
            dec.decode(b'{"a": 1, "c": 2}')

    def test_decode_dict_enum_key(self):
        dec = JSONDecoder(dict[FruitStr, int])
        ...
        with pytest.raises(
            structtype.ValidationError, match="Invalid enum value 'carrot'"
        ):
    ...
    def test_decode_dict_int_enum_key(self):
        ...
        with pytest.raises(structtype.ValidationError, match="Invalid enum value 3"):
    ...
    def test_decode_dict_int_literal_key(self):
        dec = JSONDecoder(dict[Literal[-1, 2], int])
...
class TestStruct:
    ...
    def test_decode_struct(self):
        ...
        with pytest.raises(
            structtype.ValidationError, match="Expected `object`, got `int`"
        ):
            dec.decode(b"1")
    ...
    def test_decode_struct_field_wrong_type(self):
        ...
        with pytest.raises(
            structtype.ValidationError, match=r"Expected `int`, got `str` - at `\$.age`"
        ):
            dec.decode(msg)
...
class TestStructArray:
    ...
    def test_struct_map_and_array_like_messages_cant_mix(self):
        ...
        dec = JSONDecoder(Person)
        array_dec = JSONDecoder(PersonArray)

        assert array_dec.decode(array_msg) == array_sol
        assert dec.decode(map_msg) == sol
        with pytest.raises(
            structtype.ValidationError, match="Expected `object`, got `array`"
        ):
            dec.decode(array_msg)
        with pytest.raises(
            structtype.ValidationError, match="Expected `array`, got `object`"
        ):
